...
export async function getMenus(botId: string) {
  try {
    ...
    const menus = await db
      .select()
      .from(whatsappMenus)
      .where(eq(whatsappMenus.bot_id, botId))
      .orderBy(whatsappMenus.created_at);
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Erro ao buscar menus",
    };
  }
}
